...
pub struct SimpleMenu<const NUM_LINES: usize, const LINE_SIZE: usize> {
    ...
    pub heading:      CharWriter<LINE_SIZE>,
    pub line_buf:     arrayvec::ArrayVec<CharWriter<LINE_SIZE>, NUM_LINES>,
}
...
impl<const NUM_LINES: usize, const LINE_SIZE: usize> SimpleMenu<NUM_LINES, LINE_SIZE> {
    pub fn new(posx: i32, posy: i32, max_lines: u32, name: &str) -> Self {
        let mut heading = CharWriter::<LINE_SIZE>::new();
    ...
    pub fn add_entry(&mut self, entry: &str) {
        if !self.line_buf.is_full() {
            let mut writer_entry = CharWriter::<LINE_SIZE>::new();
    ...
    pub fn draw(&mut self) {
        ...
        writer.set_position(self.posx as f32, self.posy as f32);
        self.heading.draw(&mut writer);
        let mut cursor_pos = writer.get_cursor();
        cursor_pos[0] = 0.0f32;
        writer.set_cursor(cursor_pos);
